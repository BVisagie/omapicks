...
test("advertised searches filter both lists and Enter opens the matching category", async ({ page }) => {
  await page.goto("/");
  for (const [query, category] of [["spotify", "Music"], ["weather", "Weather"], ["clipboard", "Clipboard"]]) {
    await page.getByRole("searchbox").fill(query);
    await expect(page.locator('[data-finder-item]:visible')).toHaveCount(1);
    await expect(page.locator('[data-catalog-row]:visible')).toHaveCount(1);
    await expect(page.locator('[data-finder-item]:visible')).toContainText(category);
  }
  await page.getByRole("searchbox").fill("spotify");
  await page.getByRole("searchbox").press("Enter");
  await expect(page).toHaveURL(/\/picks\/music\/$/);
});
...
test("search handles no results and restores suggestions on clearing", async ({ page }) => {
  ...
  await expect(page.locator('[data-finder-item]:visible')).toHaveCount(5);
  await expect(page.locator('[data-catalog-row]:visible')).toHaveCount(41);
});

test("comparison tables expose headers and related navigation is bounded", async ({ page }) => {
  await page.goto("/picks/network/");
...
test("copy link, install command, email and native sharing use the correct payload", async ({ page, context }) => {
  await context.grantPermissions(["clipboard-read", "clipboard-write"]);
  await page.addInitScript(() => {
    Object.defineProperty(navigator, "share", { value: async (data) => { window.shared = data; } });
  });
  await page.goto("/picks/weather/");
  await page.getByRole("button", { name: "Copy link", exact: true }).click();
  await expect(page.getByRole("status")).toHaveText("Link copied.");
  expect(await page.evaluate(() => navigator.clipboard.readText())).toBe("https://omapicks.com/picks/weather/");
  ...
  expect(email.searchParams.get("body")).toContain("https://omapicks.com/picks/weather/");
  await page.getByRole("button", { name: "Share…", exact: true }).click();
  expect((await page.evaluate(() => window.shared)).url).toBe("https://omapicks.com/picks/weather/");
});
...
test("skip link reaches main content and theme persists", async ({ page }) => {
  ...
  await page.keyboard.press("Tab");
  await expect(page.getByRole("link", { name: "Browse 41 categories", exact: true })).toBeFocused();
